...
完成了作业的必做部分，完成了选做部分的ai总结和自动发布，具体功能和介绍详见后文项目说明，演示视频和ppt在./video/目录下
...
2. 项目包含四个主要功能模块:scraper.py,process.py,summarizer.py和publisher.py
...
## publisher.py
负责自动发布./publish/目录下的图片,将其发布到小红书平台
...
3. 下一版本将尝试增加自动发布的功能
## v_1.1
1. 更正的笔记url的获取方式,修正了笔记内容显示为空的错误
2. 删除或者缩减了先前应对反爬的等待时间,理论不会遇到反爬机制拒绝访问的问题
3. 完成了自动发布的初步测试,将在下次版本更新中推送
## v_2.0
1. 添加了自动发布的功能,自动发布的测试图片在./publish/目录下
2. 由于本人账号暂时受到限制,触发二次登陆验证,所以自动登录的最终目标现在可能暂时不可用
3. 先前测试的自动登录功能是正常的,故这里认为代码实现没有错误,短期内不考虑修改这个功能的核心内容,但是添加了错误处理,用于进行手动登录
## v_2.1
1. 新增了自动启动脚本start.sh,可以在项目根目录下运行`./start.sh`即可启动项目,完成自动的爬取和处理
2. 新增了演示视频
